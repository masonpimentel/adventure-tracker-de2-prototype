...
void sync_logs(void)
{
	printf("syncing logs now\n");
	int num_logs = lastLog(0);
	printf("found %d logs\n", num_logs);
	int i;
	...
	char* end_char = "@";
	send_string_bluetooth(end_char, strlen(end_char));

}

void sync(int log_num)
{
	printf("syncing log %d\n", log_num);
	char logname[20];
	snprintf(logname, 20, "log%d.txt", log_num);
	int num_entries = numEntries(log_num);
	int fd = alt_up_sd_card_fopen(logname, false);
	if(fd < 0)
	{
		printf("sd error\n");
	}

	int dirty = check_if_dirty(fd);

	if(!dirty)
	{
		printf("log number %d has already been transfered\n");
	...
	{
		nextEntry(buf, fd);
		printf("sending log entry #%d: %s\n",i, buf);
		send_string_bluetooth(buf, strlen(buf));
		char* end_entry = "x";
		send_string_bluetooth(end_entry, strlen(end_entry));

	}

	char* end_log = "$";
	send_string_bluetooth(end_log, strlen(end_log));

	alt_up_sd_card_fclose(fd);
	int success = set_dirty_flag(logname, 0);
	if(!success)
	{
		printf("error clearing dirty flag\n");
	}
...
{
	char c = alt_up_sd_card_read(fd);
	int count = 10;
	while(c != ':' && count > 0)
	{
		count--;
		c = alt_up_sd_card_read(fd);
	}
	if(c != ':')
	{
		printf("error reading dirty flag\n");
		return -1;
	}
	c = alt_up_sd_card_read(fd);
	if(c == '1')
...
{
	short int fd = alt_up_sd_card_fopen(logname, false);
	...
	c = '0';
	int err = alt_up_sd_card_write(fd, c);
	alt_up_sd_card_fclose(fd);
	return err;
}
