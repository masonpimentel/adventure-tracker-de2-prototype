...
int DrawPath(int log)
{
	...
	while(buf[0] != '\0')
	{
		...
		else
		{
			...
			{
				FilledRectangle(current_point.x-3,current_point.y-3,current_point.x+3,current_point.y+3, RED);
			}
...
void updateCenterPixel(float latitude, float longitude, bool log_poi) {
	...
	while(i!=RPIndex){
		...
		//if((rtpCurrent.pixel.x != rtpNext.pixel.x)&&(rtpCurrent.pixel.y != rtpNext.pixel.y)){
			if(isValidPixel(rtpCurrent.pixel)&&isValidPixel(rtpNext.pixel)){
				...
				if(rtpCurrent.poi)
				{
					Rectangle(rtpCurrent.pixel.x-1, rtpCurrent.pixel.y-1,rtpCurrent.pixel.x+1, rtpCurrent.pixel.y+1, RED);
				}
...
{
	updateCenterPixel(latitude, longitude, log_poi);
	updateOtherPixels();
